...
<script lang="ts">
  ...
  const table = Table.createWithResource(imageGenerationModels);
  ...
  function listOrgs(models: ImageGenerationModel[]) {
  ...
  $: uniqueOrgs = listOrgs(imageGenerationModels);
  $: table.update(imageGenerationModels);
</script>
